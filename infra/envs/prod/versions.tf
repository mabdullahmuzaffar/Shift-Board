...
provider "azurerm" {
  ...
  features {
    ...
    key_vault {
      # Never purge in prod. Purge protection is enabled on the vault itself,
      # so a destroy would fail anyway -- this makes the intent explicit.
