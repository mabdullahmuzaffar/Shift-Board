...
variable "api_server_authorized_ip_ranges" {
  description = <<-EOT
    CIDRs allowed to reach the API server. Only used when
    private_cluster_enabled is false. Leaving this empty on a public cluster
    means the whole internet can reach the API server.
  EOT
  type        = list(string)
  default     = []
}
...
variable "private_cluster_enabled" {
  description = "Private API server. True by default in prod; needs a jumpbox or self-hosted pipeline agent inside the VNet."
  type        = bool
  default     = true
}

variable "ingress_hostname" {
  description = "Public hostname or IP of the ingress controller, used as the Front Door origin."
  type        = string
}

variable "enable_front_door" {
  description = "Provision Front Door and the WAF in front of the ingress."
  type        = bool
  default     = true
}
